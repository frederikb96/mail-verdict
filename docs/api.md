...
| Alerts | `/alerts` | The durable, in-app alert list (new mail today) and its dismiss surface, scoped to the caller's own folder preference the same way the SSE and push paths already are: `folder_ids` plus `folder_scoped=true` restricts it, omitted or `folder_scoped=false` leaves it unrestricted; `GET /alerts/vapid-public-key` and `/alerts/subscriptions` are the Web Push half — the applicationServerKey a browser passes to `PushManager.subscribe()`, and registering, updating (`alert_folder_ids`) or removing that browser's own subscription. `available: false` on the VAPID endpoint means no `ENCRYPTION_KEY` is configured server-side, the same condition that keeps a provider API key from being stored |
