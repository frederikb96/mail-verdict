...
| Messages | `/accounts/{id}/messages`, `/messages/{id}` | Cursor-paginated list (optionally threaded), detail, thread, attachment streaming, `.eml` download, single and bulk actions. `GET /{id}/quote` returns the message's raw body as safe-to-send HTML, for embedding as a reply or forward quote, or for reopening a saved draft |
