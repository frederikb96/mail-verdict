...
| Calendar events | `/calendar/events` | Month-windowed list with recurring series expanded to instances; detail; create; edit with `scope=this\|following\|all` (`following` — splitting a series in two — is rejected with `422`, not implemented; `attendees` and `tz` in an edit are also rejected with `422` rather than silently ignored, since neither is applied); delete/cancel; `POST /{id}/respond` records an RSVP and sends the `METHOD:REPLY` over the linked identity's outbox |
