...
| Contacts | `/contacts` | Paged list with address-book filter and free-text search; `/contacts/search` returns one row per email address for compose autocomplete; `/contacts/resolve?email=` returns the one contact carrying that address, or null; structured create/update/delete parsed from the vCard body, including phones, addresses, birthday, websites, categories and a photo (`kind: "embedded"` is a self-contained `data:` URI, `kind: "url"` is a third party's address that must go through the remote-content allowlist before ever being fetched) |
