...
| Mail list | `components/mail/mail-list.tsx`, `components/mail/mail-list-item.tsx`, `components/mail/bulk-panel.tsx`, `hooks/use-mails.ts`, `lib/mail-list-window.ts`, `lib/mail-intents.ts`, `lib/intent-ledger.ts`, `lib/intent-drainer.ts`, `hooks/use-mail-intents.ts` | partial | `6bc157c` | The web takes every mail action through a persisted intent ledger projected over lists, reader and counts, sends it with `idempotency_key`, and undoes it (architecture.md, "Acting on mail from the browser"). The key is a field and never deferred; the ledger, pending/failed row states and undo are the app's to mirror. |
