...
class Outbox(Base):
    ...
    # status/attempts carry no INSERT grant on this table --
    # server_default=FetchedValue() keeps a value here for ORM convenience
    # (row.status right after flush) without ever sending it explicitly on
    # INSERT, the same guard next_retry_at already uses below.
    status: Mapped[str] = mapped_column(Text, nullable=False, server_default=FetchedValue())
