...
// Below the sm breakpoint every size is the same: w-full, capped by
// max-w-[calc(100%-2rem)] so the dialog never touches the viewport edges.
// Above it, this is the whole width -- a caller used to override it with
// a plain `max-w-*` in `className`, which lost silently: Tailwind emits
// responsive-variant rules after plain ones, so `sm:max-w-sm` always won
// regardless of source order or of cn()'s own conflict resolution, which
