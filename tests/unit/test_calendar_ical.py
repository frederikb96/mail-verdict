...
# A VTIMEZONE, an X- property and CATEGORIES this codebase never models,
# plus two VALARMs -- a DISPLAY one (this application's own reminder) and
# an EMAIL one carrying an ATTACH, shaped like something another client
# wrote. Round-trip proof for the reminder edit path: none of this may be
# narrowed, dropped or regenerated by a reminder replace or an unrelated
# field edit.
_EVENT_WITH_ALARMS = (
    "BEGIN:VCALENDAR\r\n"
    "VERSION:2.0\r\n"
...
class TestReminders:
    """Reminders (DISPLAY VALARMs) and TRANSP round-trip through the edit
    path exactly the way every other field does -- mutating the parsed
    component in place, never regenerating the VEVENT from parsed fields.
    An EMAIL alarm some other client wrote is not one of this
    application's reminders and must survive untouched regardless."""

    def test_parses_a_display_reminder_and_defaults_transparency(self) -> None:
        master, _ = ical.parse_master_and_exceptions(_EVENT_WITH_ALARMS)
        assert master.reminders == [ical.Reminder(offset_minutes=-15)]
        assert master.transparency == "opaque"

    def test_replacing_reminders_preserves_the_email_alarm_and_everything_else(self) -> None:
        updated = ical.replace_master_fields(
            _EVENT_WITH_ALARMS, summary="Renamed standup",
            reminders=[ical.Reminder(offset_minutes=-30)],
        )
        master, _ = ical.parse_master_and_exceptions(updated)
        assert master.summary == "Renamed standup"
        assert master.reminders == [ical.Reminder(offset_minutes=-30)]
        ...
        alarms = [c for c in component.subcomponents if c.name == "VALARM"]
        assert len(alarms) == 2
        email_alarm = next(a for a in alarms if str(a.get("ACTION")) == "EMAIL")
        assert str(email_alarm.get("ATTACH")) == "https://example.com/agenda.pdf"
        display_alarm = next(a for a in alarms if str(a.get("ACTION")) == "DISPLAY")
        assert display_alarm.get("TRIGGER").dt == timedelta(minutes=-30)

    def test_an_absolute_reminder_round_trips(self) -> None:
        at = datetime(2026, 9, 20, 8, 0, tzinfo=timezone.utc)
        updated = ical.replace_master_fields(_EVENT_WITH_ALARMS, reminders=[ical.Reminder(at=at)])
        master, _ = ical.parse_master_and_exceptions(updated)
        assert master.reminders == [ical.Reminder(at=at)]

    def test_reminders_left_as_none_leaves_existing_alarms_untouched(self) -> None:
        """None means unchanged, the same rule every other field in
        _apply_field_overrides already follows -- reminders is not
        touched merely because some other field was edited."""
        updated = ical.replace_master_fields(_EVENT_WITH_ALARMS, summary="Renamed")
        master, _ = ical.parse_master_and_exceptions(updated)
        assert master.reminders == [ical.Reminder(offset_minutes=-15)]

    def test_setting_transparent_round_trips(self) -> None:
        updated = ical.replace_master_fields(_EVENT_WITH_ALARMS, transparency="transparent")
        master, _ = ical.parse_master_and_exceptions(updated)
        assert master.transparency == "transparent"

    def test_edit_occurrence_inherits_the_masters_reminders_via_clone(self) -> None:
        """_get_or_clone_exception() deepcopies the master, so a
        per-occurrence override starts from the master's own reminders
        rather than none at all -- a builder tidying up the clone would
        silently drop reminders on every edited occurrence. Targets the
        20261012 occurrence rather than 20260921, which already has a
        stored exception with no VALARM of its own -- editing that one
        would prove nothing about the clone path."""
        updated = ical.edit_occurrence(
            _EXOTIC_RECURRING_EVENT, "20261012T070000Z", summary="Moved again",
        )
        _, exceptions = ical.parse_master_and_exceptions(updated)
        moved = next(e for e in exceptions if e.summary == "Moved again")
        assert moved.reminders == [ical.Reminder(offset_minutes=-15)]

    def test_build_new_event_writes_reminders_and_transparency(self) -> None:
        data = ical.build_new_event(
            summary="Standalone",
            dtstart=datetime(2026, 9, 10, 9, 0, tzinfo=timezone.utc),
            dtend=datetime(2026, 9, 10, 10, 0, tzinfo=timezone.utc),
            reminders=[ical.Reminder(offset_minutes=-15), ical.Reminder(offset_minutes=0)],
            transparency="transparent",
        )
        master, _ = ical.parse_master_and_exceptions(data)
        assert master.reminders == [
            ical.Reminder(offset_minutes=-15), ical.Reminder(offset_minutes=0),
        ]
        assert master.transparency == "transparent"
