...
- An open conversation updates live: a reply arriving, or the sent copy of your own reply reaching
  ...
  whichever of its messages it was opened from. Once a message is sent (or a draft saved), the
  account is synced at once rather than on its next periodic cycle, so the Sent or Drafts copy --
  and with it the conversation -- shows up in seconds.
...
- `docs/api-contract/` holds the OpenAPI document and every SSE event name as a committed
  snapshot, regenerated by `scripts/export_api_contract.py`; CI fails when the code and the
  snapshot disagree, so every API change arrives as a reviewable diff of it.
- An SSE event name missing from the registry (`SSE_EVENT_TYPES`) is refused where it is emitted,
  and a unit test checks every name the source emits against it.
