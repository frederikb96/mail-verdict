...
- **`queue/` package:** a Postgres-native work-queue engine, parameterised by table rather than by
  ...
  every time still exhausts its attempts instead of looping forever. `pipeline_runs` is now wired
  to it (see the pipeline entry below); `message_embeddings` remains future work this is built to
  support unchanged.
